...
const CategoryDetail = ({ addToCart, favorites, toggleFavorite }) => {
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      setLoading(true);
      try {
        const catRes = await fetch(`https://tailorback2025-production.up.railway.app/api/categories/${id}`);
        const catData = await catRes.json();
        setCategory(catData);

        const prodRes = await fetch(`https://tailorback2025-production.up.railway.app/api/products?category_id=${id}`);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10 pb-32">
      {/* Header */}
      <div className="mb-12">
        <Link to="/" className="inline-flex items-center gap-2 text-red-500 hover:text-red-800 font-bold mb-6 group">
          <FaArrowLeft className="group-hover:-translate-x-1 transition-transform" />
          <span className="text-[10px] uppercase font-black tracking-widest">ASOSIY SAHIFA</span>
        </Link>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6">
            {category.children.map((child) => (
              ...
              >
                ...
                <h3 className="font-black text-[10px] uppercase italic">{child.name}</h3>
              </Link>
            ))}
          </div>
        </div>
      ...
      <div>
        ...
        {products.length > 0 ? (
              ...
              return (
                <div key={product.id} className="group flex flex-col h-full">
                  ...
                  <div className="relative aspect-[3/4] overflow-hidden rounded-[2.5rem] bg-gray-100 shadow-md">
                    ...
                    >
                      {isFav ? <FaHeart className="text-red-600" /> : <FaRegHeart className="text-gray-400" />}
                    </button>
                  </div>
                  ...
                  <div className="mt-4 px-2 flex flex-col flex-grow">
                    ...
                    <div className="mt-auto">
                      <p className="text-red-700 font-black text-xl mb-3">
                        {Math.round(product.price).toLocaleString()} <span className="text-xs">SO'M</span>
                      </p>
                      
                      {/* --- SAVAT TUGMASI (DOIM KO'RINADIGAN VA CHIROYLI) --- */}
                      <button 
                      ...
                      >
                        <FaShoppingCart className="group-hover/btn:animate-bounce" size={16} />
                        <span className="font-black text-[11px] uppercase tracking-tighter">Savatga qo'shish</span>
                      </button>
                    </div>
                  </div>
                </div>
          ...
          <div className="py-20 text-center bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-200">
            <p className="text-gray-400 font-black italic">MAHSULOTLAR TOPILMADI</p>
          </div>
        )}
      </div>
    </div>
